...
const ParticleBackground = () => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            asteroids.forEach(asteroid => {
                ...
                // Wrap around screen
                if (asteroid.x < -20)
                    asteroid.x = canvas.width + 20;
                if (asteroid.x > canvas.width + 20)
                    asteroid.x = -20;
                if (asteroid.y < -20)
                    asteroid.y = canvas.height + 20;
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "fixed top-0 left-0 w-full h-full -z-10 bg-[#1A1F2C]" }));
};
